	...
	string obtemp = "";
 	...
 	cout << "If you wanna end the program. Input e ..." << endl << "MIPS : " << endl << "_________________________________________" << endl << endl;
  	...
  	while(getline(cin, my_string, '\n')) {
  		...
  		if(cheak_read == true){								//�p�G��Ū���줺�e 
	  		string line[6] = "";
	    	char *pch;
	    	char str[128];
	    	int i=0;
	   		int leave = 0;									
	   		correct = true;									//�W�L�@��̦h���J���j�p �X�� 
	...
	if(line[1] == ""){								//label
		...
		for(int i=0 ; i<12 ; i++){					//find label index
			if(line[0] == op_table[i].name){
				com_index = i;						//get command index (�boptable��txt������) 
		...
		if(line[0] == com_name){
			...
			if(com_name == "add" || com_name == "sub" || com_name == "slt"){		
				...
				for(int i=0 ; i<32 ; i++){				
					if(line[3] == reg_table[i].name){					//rt
						obcode += ten_to_binary(reg_table[i].value, 1);
					}
				}	
				for(int i=0 ; i<32 ; i++){	
					if(line[1] == reg_table[i].name){					//rd
						obcode += ten_to_binary(reg_table[i].value, 1);
					}		
				}
				...
				if(com_name == "addi" || com_name == "slti"){			//imm
					obcode += ten_to_binary(line[3], 2);
					if(line[3] == "1"){									//if imm = 1 
						obcode += "1";
					}
				}
			}
			else if(com_name == "lw" || com_name == "sw"){				
				for(int i=0 ; i<32 ; i++){								//rs
					if(line[3] == reg_table[i].name){					
						obcode += ten_to_binary(reg_table[i].value, 1);
					}
				}
				for(int i=0 ; i<32 ; i++){								//rt
